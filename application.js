...
jQuery(function($) {
  ...
  function showPlace (place) {
    ...
    placesService.getDetails({
      ...
      if (data.address_components.length < 3) {
        $('#fucking-next-location').trigger("click");
        return;
      };
      if (frontPage == true) { toggleFuckingView(); };
  ...
  $('#fucking-next-location').click(function(event){
    ...
    if (currentPlace == places.length) {
      if (currentPlace >= 20 || currentRadius >= 64000) {
        currentPlace = 0;
      }else{
        currentRadius *= 1.5;
        findPlaces();
    ...
    places[currentPlace] && showPlace(places[currentPlace]);
  });
